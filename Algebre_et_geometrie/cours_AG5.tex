...
\begin{center}
	...
	\coordinate (c) at (180:2.5);
	\coordinate (d) at (270:2.5);
	
	\node (G) at (45:3.3) {$G$};
	\node (K1) at (0:1.5) {$K_1$};
	\node (K2) at (90:1.5) {$K_2$};
	\node (K3) at (180:1.5) {$K_3$};
	\node (K4) at (270:1.5) {$K_4$};
	
	\draw[fill=black] (e) circle (0.05) node[above right] {$e$};
	\draw[fill=black] (45:2.5) circle (0.05);
	\draw[fill=black] (125:2.5) circle (0.05);
	\draw[fill=black] (145:2.5) circle (0.05);
	\draw[fill=black] (10:2) circle (0.05);
	\draw[fill=black] (80:2) circle (0.05);
	\draw[fill=black] (100:2) circle (0.05);
	\draw[fill=black] (170:2) circle (0.05);
	\draw[fill=black] (190:2) circle (0.05);
	\draw[fill=black] (260:2) circle (0.05);
	\draw[fill=black] (280:2) circle (0.05);
	\draw[fill=black] (350:2) circle (0.05);
	\ellipsebyfoci{draw}{e}{a}{1.3}
	\ellipsebyfoci{draw}{e}{b}{1.3}
	\ellipsebyfoci{draw}{e}{c}{1.3}
	\ellipsebyfoci{draw}{e}{d}{1.3}
	\end{tikzpicture}
\end{center}
...
\begin{remar}
Soit $A$ un $p$-groupe abélien, $|A|=p^m=p^{m_1}\times\ldots\times p^{m_k}$.\\
On a donc $m=\sum_{i=1}^k m_i$. Supposons $m_1 \geq m_2 \geq \ldots \geq m_1$.\\
La suite des $m_i$ est alors une \textbf{partition} de $m$, , i.e. elle est
d\'ecroissante et la somme des termes est $m$.\\
Donc les $p$-groupes abéliens d'ordre $p^m$ sont classifiés par les partitions de $m$.\\
De façon équivalente, ils ont classifiés par les \textbf{diagrammes de Young} à $m$ cases.
\end{remar}
...
\begin{align*}
p^r :&\Z/p^l \rightarrow \Z/p^l\\
&\bar{k} \mapsto p^r\bar{k}
\end{align*}
Si $r \geq l$, alors le noyau est $\Z/p^l$ tout entier.\\
Si $r < l$, alors le noyau est le \sg engendré par $\overline{p^{l-r}}$. Dans ce cas, il est donc d'ordre $p^r$.\\
Donc :
$$\abs{\ker(p^r : \Z/p^l \rightarrow \Z/p^l)}=\left\{\begin{array}{ll}
p^l& si~r\geq l\\
p^r& si~r \leq l
\end{array}\right.$$
...
$$p^r\cdot (b,c)=(p^rb, p^r c)= (0,0)\Leftrightarrow p^rb=0~\text{et}~p^rc=0$$
On a $A\cong \Z/p^{m_1}\oplus \ldots \oplus \Z/p^{m_k}$. Le noyau de $p^r:A\rightarrow A$ est la somme directe des noyaux de $p^r:\Z/p^{m_i}\rightarrow \Z/p^{m_i}$. Donc :
$$\abs{\ker(p^r : A \rightarrow A)}=\prod_{m_i\leq r}p^{m_i}\prod_{m_i> r}p^r = p^{O_r}$$
avec $O_r=\sum_{m_i\leq r}m_i+\sum_{m_i> r}r$.\\
Montrons que la suite des nombres $O_r$ détermine la tableau de Young associé à $m_1\geq \ldots \geq m_k$.\\
...
Donc les $O_r$ déterminent le diagramme de Young et la suite $m_1\geq \ldots \geq m_k$.\\
\textbf{Existence :} Par récurrence sur $|A|$. Si $|A|=1$, alors $A=\{0\}$ et il n'y a rien à démontrer (la suite des $m_i$ est vide).\\
Supposons $|A|>1$. Soit $a\in A$ d'ordre maximal. Par l'hypothèse de récurrence, on a un \isom :
$$\varphi : \Z/q_2 \oplus \ldots \oplus \Z/q_k\fong A/\gen{a}$$
pour certaines puissances $q_i=p^m_i$, $2\leq i \leq k$. Notons :
$$e_i=(\bar{0},\ldots,\bar{1},\ldots, \bar{0})\in \Z/q_2 \oplus \ldots \oplus \Z/q_k \text{   (}\bar{1} \text{ à la i-ème position)}$$
...
\red{figure by tristan Francois}

On définit :
$$
\begin{array}{lrl}
\psi :& \Z/q_2 \oplus \ldots \oplus \Z/q_k &\rightarrow A\\
&(\overline{j_i},\ldots, \overline{j_k})&\mapsto \sum_{l=2}^kj_lx_l
\end{array}
$$
Comme $x_l$ est d'ordre $q_l$, $\psi$ est bien défini. Par construction, on a $\pi \circ \psi = Id_{A/\gen{a}}$ car $\pi \circ \psi(e_i)=\pi(x_i)=\varphi(e_i)$ et les $e_i$ engendrent $\Z/q_2 \oplus \ldots \oplus \Z/q_k$.\\
Donc $\psi$ est injectif. Donc $\psi$ est un \isom de $\Z/q_2 \oplus \ldots \oplus \Z/q_k$ sur $Im(\psi)$. Comme $\pi\circ \psi$ est surjectif, on a $Im(\psi)+\gen{a}=A$. Comme $\pi \circ \psi$ est injectif, on a $Im(\psi)\cap\gen{a}=\{0\}$.\\
Soit $q_1$ l'ordre de $a$. Alors $\Z/q_1\fong \gen{a}$, $\bar{1}\mapsto a$. Donc $A=\gen{a}\oplus Im(\psi)\cong \Z/p_1 \oplus \Z/q_2 \oplus \ldots \oplus \Z/q_k$.
\end{demo}
...
\begin{exem}
Soit $D\subseteq \R^2$ une droite qui \textbf{ne passe pas par l'origine} :

\red{figure 2 de tristan francois}
...
\begin{definition}
Un \slign{espace affine} sur $K$ est un triplet 
\gray{$(\mathcal{E}, \; E, \; \phi)$}, où
\begin{itemize}[label=$\bullet$]
\item $\mathcal{E}$ est un ensemble non vide

\item $E$ est un $\K$-espace vectoriel

\item $\phi : \mathcal{E} \times E \longrightarrow \mathcal{E}$ est une action \textbf{libre et transitive} du groupe $(E, +)$ sur l'ensemble $\mathcal{E}$.
\end{itemize}
\end{definition}

\begin{remarque}
Dans la situation de la définition, on abrège $(\mathcal{E}, E, \phi)$ en $\mathcal{E}$ et on dit que $\mathcal{E}$ est un \textbf{espace affine de direction} $E$.
Les éléments de $\mathcal{E}$ sont appelés \slign{points}, ceux de $E$ \slign{vecteurs}.
\end{remarque}

\begin{exemple}
Soit $E$ un $\K$-espace vectoriel. Soit $\mathcal{E} = E$, et 
$$\phi: \mathcal{E} \x E \rightarrow \mathcal{E},~(P,v)\mapsto P+v $$
...
\[
\phi(P, v) = P + v,		\qquad	\forall P \in \mathcal{E}, \forall v \in E
\]
\end{notation}
...
\item $\forall P,Q \in \mathcal{E},~\exists ! u \in E$ tel que $Q = P + u$ (l'action est libre et transitive)
\end{itemize}
\end{remarque}

\begin{notation}
Pour tous $P, Q \in \mathcal{E}$, on note $\vec{PQ}$ l'unique vecteur tel que
\[
Q = P + \vec{PQ}
\]
\end{notation}
...
\item $\vec{PQ} + \vec{QR} = \vec{PR} \quad$ \gray{(relation de Chasles)}

\red{figure 3 tristan francois}

\item $P + u = Q + v		\quad	\Leftrightarrow	\quad		\vec{PQ} = u - v$
\red{figure 4 tristan francois}
\end{itemize}
\end{remarque}
...
\begin{definition}
Un sous-espace affine de $\mathcal{E}$ est une partie $\ensF \subseteq \mathcal{E}$ telle qu'il existe un point $P \in \mathcal{E}$ et sous-espace vectoriel $F \subset E$ tel que
\[
\ensF = \Ens{P + v \big| v \in F} = P + F
\]

\red{figure 5 tristan francois}
\end{definition}

\begin{exemple}
\begin{enumerate}
\item  $\ensF = \mathcal{E}$ est un sous-espace affine \gray{($F = E$)}

\medbreak

\item  Pour tout point $P \in \mathcal{E}$, le singleton $\ens{P}$ est un sous-espace affine de direction $\ens{0} \subseteq E$.

\medbreak

\item Soit  $P\in \R^2$ un point et $\vec{D}\subseteq \R^2$ une droite vectorielle. Alors :
$$D=P+\vec{D}$$
est un sous-espace affine de $\R^2$ considéré comme espace affine.

\red{figure 6 tristan francois}
\end{enumerate}
\end{exemple}

\begin{remarque}
Soient $\mathcal{E}$ un espace affine, $P \in \mathcal{E}$ et $F \subset E$ un sous-espace vectoriel.\\
Posons $\ensF = P + F$.
\begin{enumerate}
\item On a $F = \Ens{\vec{QR} \big| Q, R \in \ensF}$.\\
Donc $F$ est \textbf{uniquement} déterminé par $\ensF$

\item $\forall Q \in \mathcal{E}		\qquad	Q \in \ensF		\quad	\Leftrightarrow	\quad	\vec{PQ} \in F$
...
\begin{definition}
Soit $\ensF \subseteq \mathcal{E}$ un sous-espace affine
\begin{enumerate}
\item L'unique sous-espace vectoriel $F$ de $E$ tel que $\ensF = P + F$, $\forall P \in \ensF$, est appelé la \slign{direction} de $\ensF$ et parfois noté $\vec{\ensF}$

\item On appelle \slign{dimension} de $\ensF$ la dimension de $F$

\item Si $\dim(\ensF) = 1$ \gray{(resp. 2)}, on dit que $\ensF$ est une \slign{droite affine} \gray{(resp. un \slign{plan affine})} de $\mathcal{E}$
\end{enumerate}
\end{definition}

\begin{exemple}
\begin{enumerate}
\item  On suppose que $\dim(E) = 2$. Soit $e_1, e_2$ une base de $E$. soit $P \in \mathcal{E}$ et soit $u \in E \backslash \ens{0}$.

Soit $\ensD$ la droite affine $P + \Vect(u)$. On a
\[
\forall M \in \mathcal{E} :	\qquad		M \in \ensD \Leftrightarrow	\quad	\det_{(e_1, e_2)}(u, \vec{PM}) = 0
\]

\medbreak

\item  On suppose que $\dim(E) = 3$. Soit $e_1, e_2, e_3$ une base de $E$. Soit $P \in \mathcal{E}$ et soient $u, v \in E$ linéairement indépendants.\\
On note $\ensP$ de plan affine $P + \Vect(u, v)$.\\
On a:
\[
\forall M \in \mathcal{E}	\qquad		\det_{(e_1, e_2, e_3)}(u, v, \vec{PM}) = 0
\]
\end{enumerate}
\end{exemple}
...
\item $\mathcal{E}$ est l'unique sous-espace affine de direction $E$

\medbreak

\item  Les sous-espaces affines de direction $\ens{0}$ sont exactement les singletons de $\mathcal{E}$
\end{enumerate}
\end{remarque}

\begin{exemple}
\begin{enumerate}
\item  $\Ens{(x, y, z) \in \K^3 \big| x + y + z = 1}$ est un sous-espace affine de $\mathcal{E} = \K^3$.\\
C'est un plan de direction
\[
\Ens{(x, y, z) \in \K^3 \big| x + y + z = 0} \subseteq E = \R^3
\]

\red{figure 7 tristan francois}

\item  Soit $n \in \N$. Alors $\ensF = \Ens{(x_1, \ldots, x_{n+1}) \in \K^{n+1} \big| x_1 + \ldots + x_{n+1} = 1}$ est un sous-espace affine de $\mathcal{E} = \K^{n+1}$ de dimension $n$ et de direction
\[
...
\]
est un sous-espace affine de $\mathcal{E} = V$ de direction $\Ker(f) \subset E = V$. Si $\dim(V) < \infty$, $\ensF$ est de dimension $\dim(V) - 1$. On dit que $\ensF$ est un \slign{hyperplan affine} dans $\mathcal{E} = V$.

\item Soit $f:V\rightarrow W$ une application linéaire et $w\in W$. Supposons qu'il existe $P_0 \in V$ tel que $f(P_0)=w$ et soit :
$$\ensF = \{P\in V \big| f(P)=w\}\subseteq V=\mathcal{E}$$
Alors $\ensF$ est un sous-espace affine de direction $\ker(f)$.\\
En effet, on a :
$$P\in \ensF \Leftrightarrow f(P)=w=f(P_0)\Leftrightarrow f(\vec{P_0P})=0 \Leftrightarrow P=P_0+v\quad v\in \ker(f)$$
Donc $\ensF = P_0+\ker(f)$.

\item Soit $A\in M_{n,p}(\R)$ et $b\in \R^n$. Considérons le système inhomogène :
$$A\cdot x = b\quad x\in \R^p$$
Soit $\ensF = \{x \in \R^p \big| A\cdot x = b \}$. Alors ou bien $\ensF$ est vide, ou bien $\ensF$ est non vide.\\
Supposons $\ensF$ non vide et soit $x_{part}$ une solution du système inhomogène, alors on sait que :
\begin{align*}
\ensF &= \{x_{part} + x_{hom} \big|~Ax_{hom}=0 \}\\
&=x_{part}+\ker(A)
\end{align*}
Donc $\ensF \subseteq \R^p$ est un sous-espace affine de direction $\ker(A)$.
\end{enumerate}
\end{exemple}
