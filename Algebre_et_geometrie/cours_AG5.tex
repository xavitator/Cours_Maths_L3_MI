...
\begin{remar}
Soit $A$ un $p$-groupe abélien, $|A|=p^m=p^{m_1}+\ldots+p^{m_k}$.\\
On a $m=\sum_{i=1}^k m_i$. La suite des $m_i$ est une partition de $m$.\\
...
p^r& si~r \leq m
\end{array}\right.$$
Si $B$ et $C$ sont deux groupes abéliens, alors le noyau de $p^r:B\oplus \rightarrow B\oplus C$ est la somme directe des noyaux de $p^r:B\rightarrow B$ et de $p^r:C\rightarrow C$, car :
...
\item $\phi : \varepsilon \times E \longrightarrow \varepsilon$ est une action \textbf{libre et transitive} du groupe $(E, +)$ sur l'ensemble $\varepsilon$.
\end{itemize}
\end{definition}

\begin{remarque}
Dans la situation de la définition, on abrège $(\varepsilon, E, \phi)$ en $\varepsilon$ et on dit que $\varepsilon$ est un \textbf{espace affine de direction} $E$.\\
Les éléments de $\varepsilon$ sont appelés \slign{points}, ceux de $E$ \slign{vecteurs}.
\end{remarque}

\begin{exemple}
Soit $E$ un $\K$-espace vectoriel. Soit $\varepsilon = E$, et 
$$\phi: \varepsilon \x E \rightarrow \varepsilon,~(P,v)\mapsto P+v $$
Alors $(\varepsilon, E, \phi)$ est un espace affine appelé l'espace affine associé à l'espace vectoriel $E$. Sa direction est $E$.
\end{exemple}
...
\item $\forall P,Q \in \varepsilon,~\exists ! u \in E$ tel que $Q = P + u$ (l'action est libre et transitive)
\end{itemize}
\end{remarque}
...
\item  Pour tout point $P \in \varepsilon$, le singleton $\ens{P}$ est un sous-espace affine de direction $\ens{0} \subseteq E$.
...
\begin{remarque}
Soient $\varepsilon$ un espace affine, $P \in \varepsilon$ et $F \subset E$ un sous-espace vectoriel.\\
Posons $\ensF = P + F$.
\begin{enumerate}
...
\begin{definition}
Soit $\ensF \subseteq \varepsilon$ un sous-espace affine
\begin{enumerate}
...
$$\ensF = \{P\in V \big| f(P)=w\}\subseteq V=\varepsilon$$
