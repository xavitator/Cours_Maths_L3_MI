...
\begin{exem}
	...
	C'est bijectif : sa réciproque est $c_g^{-1}$ car $c_g^{-1}(c_g(x))=x$ $\forall x\in G$ et $c_g(c_g^{-1}(x)) = x$ $ \forall x\in G$\\
...
\begin{exem}
	\begin{enumerate}
		...
		Ici, comme dans tout les diagrammes de groupes diédraux à venir, les " \tikz[baseline=-0.5ex]\draw [dashed] (0,0) -- (0.6,0);" représentent la relation de conjugaison par $\rho$.
...
On a :
$ N_{\A_4}(\gen{(1~2)(3~4)})=V$ est d'indice 3 dans $\A_4$\\
$ N_{\A_4}(\gen{(1~2~3)})=\gen{(1~2~3)}$ est d'indice 4 dans $\A_4$
\end{enumerate}
\end{exem}
...
\begin{remar}
On montrera que tout diviseur premier $p$ de $\abs{G}=n$, il \textbf{existe} un $p$-Sylow, et que tous les $p$-Sylow sont \textbf{conjugués}.
\end{remar}
...
\begin{remarque}
Notons que comme $N$ est distingué, $K \cdot N$ est un sous-groupe et en fait égal à $\vec{K, N}$
\end{remarque}
