...
Soit $(X,d)$ avec $d:X^2\rightarrow \R+$ appelé distance.
...
\end{enumerate}
C'est une \ev métrique.


\subsubsection{Définition}
$n$ est dite semi-normé sur E (\ev de $\R$) ssi n vérifie 2) et 3) de la définition 1.
...
\subsubsection{Définition}
Soit V une partie de E, N ou X, d est un voisinage de $x_0\in E~(ou~X)$ ssi il existe U ouvert contenant $x_0$ et contenu dans V.
...
\subsubsection{Définition}
On appelle espace topologique $(X, P)$ un ensemble muni d'une famille de parties $T$ ($\subset P(x)$) dites ouvertes qui vérifie :
\begin{enumerate}
\item $\emptyset$, $X\in T$
\item $\cup_{\alpha \in J}U_\alpha \subset T$, si $\forall \alpha\in J$ $U_\alpha \in T$
\item $\cap_{i\in I}U_i\subset T$, si $\forall i\in I$ (I de cardinal fini) $U_\alpha\in T$
\end{enumerate}
...
\subsubsection{Définition}
On dit que $N_1$ et $N_2$ deux normes sur un même \ev E sont équivalentes ssi :
...
\begin{enumerate}
\item $\supseteq$ : Si $a\in A$ et $\exists r>0~B(a,r)\subset A$, alors $B(a,r)$ est ouverte, et donc $a\in \cerc{A}$\\
$\subseteq$ : Si $a\in \cerc{A}$, $\exists U$ ouvert, $U\subset A$, avec $a\in U$. Donc $\exists~r>0$ $B(a,r)\subset U$ ($U$ est ouvert selon une topologie associée à la norme / distance)

\item Si $a\in A$, la propriété est claire.\\
On a :
$$E\backslash \bar{A} = \left\{x\in E;~\exists r>0~ B(x,r) \cap A = \emptyset \right\} \subset E\backslash A$$

Si $x$ vérifie $B(x,r)\subset X\backslash A$, alors $\exists F=X\backslash B(x,r)$ (boule fermée) tq $x\notin F$ (avec $F\supset A$), donc $x\in \bar{A}$.\\
Si $x\in X\backslash \bar{A}$, $\exists U \subset X\backslash \bar{A}$ tq $x\in U$.\\
...
Si $(E,N)$ est un \ev normé, alors $\bar{F}$ (où F est un \sev) est un \sev.

\subsubsection{Démonstration}

Soit $(E,N)$ et $F$ un \sev de E, on a $\bar{F}$ est fermée.\\
$\bar{F}$ est non-vide car $\bar{F}\supset F$, il reste donc à montrer que $\bar{F}$ est stable par combinaison linéaire :\\
Soit $x\in \bar{F},\lambda \in \R$, on a :
$$\begin{array}{lll}
B(\lambda x, r)&=& \{y,N(y-\lambda x)<r \}\\
&=& \{z=\frac{y}{\lambda};~|\lambda |N(z-x)<r \}\\
&=& \{z=\frac{y}{\lambda};~N(z-x)<\frac{r}{|\lambda |} \}
\end{array}$$
Donc $\exists z\in F$ tq $z\in B(x,\frac{r}{|\lambda |})$\\

Soit $r>0$, et $x,y\in \bar{F}$ :\\
$\exists z\in B(x,\frac{r}{2})~\exists t\in B(y,\frac{r}{2})$ par l'inégalité triangulaire.\\
$z+t\in B(x+y,r)$ et $N(z+t-x-y)< N(z-x)+N(t-y)<r$

\subsubsection{Remarque}
\begin{enumerate}
\item La propriété 1 est fausse si $(X,d)$ est métrique (pas vrai dans tous les cas)
...
Le membre de droite est le reste de la série de Riemann convergente, il converge donc vers 0.\\
...
si une famille finie de fermés emboîtés qui est vide, un est vide.

\subsubsection{Contraposée}
Si $\forall n$ $F_n\neq \emptyset$ alors $\cap_n F_n \neq \emptyset$

\subsubsection{Proposition 1}
Si $K$ est compacte par $(X,d)$ $(E,N)$ $(X,C)$, alors $K$ est fermée.

\subsubsection{Note}
Tout point $y \notin K$ peut être séparé de $K$ (c'est à dire $\exists U\supseteq K$ $\exists V$ avec $y\in V$ tq $U,V$ sont ouverts et $U\cap V = \emptyset$).
