...
$\partial f_a \in \L(\R^n, \R^p)$ et :
\begin{align*}
\partial f_a (h_1,\ldots, h_n)&=(\partial f_1(a) (h_1,\ldots, h_n),\ldots, \partial f_p(a) (h_1,\ldots, h_n))
&=(h_1\frac{\partial f_1}{x_1}(a)+\ldots + h_n\frac{\partial f_1}{x_n}(a), \ldots)
\end{align*}
On peut y associer une matrice, la matrice Jacobienne ($J_{f_a}$) avec $\frac{\partial f_i}{\partial x_j}$ avec $i$ la ligne et $j$ la colonne.\\
...
Souvent appliqué avec $f'$ continue sur $[a,b]$, $i.e$ $f\in C^1([a,b])$.
\end{thm}

\begin{thm}[Énoncé 3 : Formule de Taylor avec reste intégrale]
Soit $f\in C^1([a,b])$, on a :
$$f(b)-f(a)=(b-a)\int_0^1f'(a+s(b-a))ds$$
\end{thm}

Soit $f:\U \subseteq E \rightarrow \R$.\\
...
$$\exists c \in [a,b]~tq~f(b)-f(a)=\partial f_c(b-a)$$
\end{propo}
...
\item Si $f$ est $C^1$ ($i.e$ $f$ est continue, différentiable en tout point de $\U$ et que sa différentielle est continue sur $\U$) alors $\sup_{c \in [a,b]}\norm{df_c} \leq M$, pour $M>0$.
\end{enumerate}
\end{remar}
...
est une application différentiable en $a$ alors on dira que $f$ est différentiable à l'ordre $2$.\\
...
Puis par récurrence, on est $C^n$ et enfin $C^\infty$ (si $\forall n \in n$ $C^n$).
\end{defi}
...
Soit $f$ de classe $\C^2$ sur le segment $[a,b]$ ($b=a+h$) dans un Banach alors :
...
$$N_F(f(a+h)-f(a)-df_a(h)) \leq M N_E(h)^2$$
Et à l'ordre $n$ :
...
\begin{propo}
Si $f$ est différentiable en $a$ alors $df_a=0$
\end{propo}

\begin{propo}
Si $f$ est différentiable en $a$ à l'ordre 2, alors $df_a = 0$ et $d^2f_a(h,h) \geq 0$ (minimum local) ou $d^2f_a(h,h) \leq 0$ (maximum local).\\
...
\begin{propo}
Soit $g(t)=f(a+th)-f(a)-tdf_a(h)-t^2d^2f_a(h,h)-t^2N_E(h)^2\epsilon (th)$. On a :
...
\begin{remar}
Si $E=\R^n$ alors $B(u,u)\geq mN_E(u),~m>0$.
\end{remar}
